...
--
-- The abs() switch makes the bound non-monotone below -25: a node there can
-- bound lower than its own successor, which would break the assumption that
-- a parent's bound caps every descendant. Pruning relies on that assumption.
-- Audited: the bound is monotone and the lineage property exact for
-- v >= -24.5; it only fails at v <= -25. This search is not expected to go
-- below about -5, so the piecewise model is sound here with ~5x margin.
-- BOUND_SAFE_MIN_VEL guards the assumption at runtime rather than trusting
-- it; if the warning ever fires, set ACCEL_LOW = ACCEL_HIGH, which is
-- unconditionally sound at every velocity.
...
-- Always <= score_at alone, so it is a strict tightening. Monotone in
-- velocity above BOUND_SAFE_MIN_VEL, so a parent's bound is a valid upper
...
-- Guard on the piecewise bound's validity domain. Warns once.
...
local function replay(seq)
    ...
    for i = 1, #seq do
        ...
        -- gui.text(4, 32, string.format("score: %.3f",
        --     accel_bound(x_pos, x_vel_ub, i - 1)))
        ...
        if x_vel < min_vel_seen then
            ...
            if x_vel < BOUND_SAFE_MIN_VEL and not vel_warned then
                ...
                emu.print(string.format(
                    "WARNING x_vel=%d below %d: piecewise bound may be non-monotone here."
                    .. " Set ACCEL_LOW = ACCEL_HIGH. | %s",
                    x_vel, BOUND_SAFE_MIN_VEL, table.concat(seq, ",")))
            end
        end
...
local function search(max_depth)
    ...
    while true do
        ...
        -- Pre-replay prune. inherited_bound is the parent's bound, which is
        -- an upper bound on every descendant. The incumbent may have risen
        -- while this node sat in the frontier, so check before paying for a
        -- replay that grows with depth.
        if node.inherited_bound and node.inherited_bound <= best_score then
            ...
            if evaluated % PROGRESS_EVERY == 0 then
                emu.print(string.format("[%d] pruned=%g skipped=%g capped=%g best=%s | d=%d %s",
                    evaluated, pruned_count, skipped_replays, capped_count,
                    best_score_str(), node.depth, table.concat(node.seq, ",")))
            end
            ...
            elseif reached_cap then
                -- At cap the remainder is determined by score_at; further
                -- frames cannot reveal anything new.
    ...
    emu.print(string.format(
        "evaluated=%d pruned=%d (replays skipped=%d) capped=%d min_vel=%d (full tree %g)",
